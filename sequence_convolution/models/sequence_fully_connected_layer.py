...
class SequenceFullyConnectedLayer(HiddenLayer):
    n_parameters = 2

    def __init__(self, n_in, n_filters, activation_function='sigmoid',
                 weights_scale=.01, W=None, b=None,
                 l1_penalty_weight=0., l2_penalty_weight=0.,
                 dtype=np.float32):
        if W is None:
            self.W = weights_scale * \
              sampler.gen_uniform((n_filters, 4 * n_in), dtype=dtype) \
              -.5 * weights_scale
        else:
            self.W = W
        ...
        assert self.W.shape == (n_filters, 4 * n_in)
        assert self.b.shape == (n_filters,)

        self.n_in = n_in
        self.n_filters = n_filters
        self.n_units = n_filters
    ...
    def feed_forward(self, input_data, prediction=False):
        activations = \
            pycuda_ops.fully_connected_layer(input_data, self.W, self.b)
    ...
    def backprop(self, input_data, df_output, cache=None):
        if cache is None:
            activations = self.feed_forward(input_data)[0]
        else:
            activations = cache[0]
        ...
        df_W = pycuda_ops.fully_connected_layer_gradient(
            input_data, delta)
