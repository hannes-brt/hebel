...
__global__ void conv1d(%(data_type)s *input, %(data_type)s *target, %(data_type)s *filter,
    int width, int filter_width) {
    
    int i = blockIdx.x*blockDim.x+threadIdx.x;
    __shared__ %(data_type)s input_shared[TILE_SIZE+MAX_WIDTH_FILTER-1];
    
    int halo_width = filter_width / 2;
    
    int halo_index_left = (blockIdx.x-1)*blockDim.x+threadIdx.x;
    if (threadIdx.x >= blockDim.x-halo_width) {
        input_shared[threadIdx.x-(blockDim.x-halo_width)] = 
            (halo_index_left < 0) ? 0 : input[halo_index_left];
    }
    
    input_shared[halo_width+threadIdx.x] = input[i];
    
    int halo_index_right = (blockIdx.x+1)*blockDim.x+threadIdx.x;
    if (threadIdx.x < halo_width) {
        if (width >= TILE_SIZE) {
            input_shared[halo_width+blockDim.x+threadIdx.x] =
                (halo_index_right >= width) ? 0 : input[halo_index_right];
        } else {
            input_shared[halo_width+width+threadIdx.x] = 0;
        }
    }
    ...
    for (int j=0; j < filter_width; j++) {
        Pvalue += input_shared[threadIdx.x+j]*filter[j];
    }
    if (i < width)
        target[i] = Pvalue;
...
__global__ void conv1d_matrix(%(data_type)s *input, %(data_type)s *target, %(data_type)s *filter,
    unsigned int width, unsigned int height, unsigned int filter_width) {
    ...
    if (i < height && j < width) {
        for (int k=0; k < filter_width; k++) {
            ...
            Pvalue += input_shared[shared_idx]*filter[k];
        }
        if (j < width)
            target[lin_idx] = Pvalue;
    }
}

__global__ void conv1d_matrix_mult_filter(%(data_type)s *input, %(data_type)s *target, %(data_type)s *filter,
    unsigned int width, unsigned int height, unsigned int filter_width, unsigned int n_filters) {
    ...
    if (i < height && j < width) {
        for (int f=0; f < n_filters; f++) {
            ...
            target_idx = f*width*height+i*width+j;
...
conv1d_kernel_float = source_module_float.get_function('conv1d')
conv1d_matrix_kernel_float = source_module_float.get_function('conv1d_matrix')
conv1d_matrix_mult_filter_kernel_float = source_module_float.get_function('conv1d_matrix_mult_filter')
conv1d_kernel_double = source_module_double.get_function('conv1d')
conv1d_matrix_kernel_double = source_module_double.get_function('conv1d_matrix')
conv1d_matrix_mult_filter_kernel_double = source_module_double.get_function('conv1d_matrix_mult_filter')

def conv1d_matrix(mat, conv_filter, target=None, stream=None):
    dtype = mat.dtype
    ...
    width_filter = conv_filter.size

    if target is None:
        target = gpuarray.empty_like(mat)

    if dtype == np.float32:
        kernel = conv1d_matrix_kernel_float
    elif dtype == np.float64:
        kernel = conv1d_matrix_kernel_double
    else:
        raise ValueError
...
def conv1d_matrix_mult_filter(mat, conv_filter, target=None, stream=None):
    dtype = mat.dtype
    assert dtype in (np.float32, np.float64)
    assert conv_filter.dtype == dtype
    ...
    if target is None:
        target = gpuarray.empty((n_filters, height, width),
                                dtype=dtype)
